...
Current state: `0.1.1` planning policy foundation release plus production-depth policy persistence slice. The module includes deterministic sample checks, optional database-backed policy and staff-analysis records, `civiccore==0.3.0` alignment, and a public sample UI at `/civicplan`. It does not provide legal advice, official planning determinations, live GIS, live LLM calls, plan document ingestion, permitting-system integrations, or final staff-report approval.

## For IT And Technical Staff

CivicPlan is a FastAPI Python package pinned to `civiccore==0.3.0`. The current runtime exposes:
...
flowchart LR
  ...
  CivicPlan --> CivicCore["CivicCore v0.3.0"]
  CivicPlan -. released-context .-> CivicZone["CivicZone v0.1.1"]
  CivicPlan -. released-context .-> CivicClerk["CivicClerk v0.1.0"]
```

CivicPlan depends on CivicCore. CivicCore does not depend on CivicPlan. CivicPlan v0.1.1 uses deterministic sample policy data only; official plan ingestion and cross-module APIs are future work.
