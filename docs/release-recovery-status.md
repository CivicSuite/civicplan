...
| Runtime install proof | Passing in branch | Fresh WSL editable install succeeded with the published CivicCore v1.0.0 release wheel and Hatch direct references enabled. |
...
| Mock-vs-production labeling | Passing in branch | Existing docs distinguish local adversarial mocks, no legal advice, no official planning determinations, and no live external calls by default. |
| Browser/user-flow QA | Passing in branch | Playwright checked `docs/index.html` at desktop and mobile widths; see `docs/browser-qa-civicplan-release-recovery-summary.md`. |

## Current Evidence

- Focused Windows regression tests: `7 passed`.
- Native WSL release gate: `33 passed`; docs gate, placeholder import gate,
